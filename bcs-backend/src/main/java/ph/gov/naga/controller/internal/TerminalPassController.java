...
public class TerminalPassController {
    ...
    public ResponseEntity<?> delete(@PathVariable("id") long id) {
        ...
        if (savedRow == null) {
            logger.error("Unable to find TerminalPass vehicle with id {} ", id);
            return new ResponseEntity(HttpStatus.NOT_FOUND);
        }
        //prevent deletion if payment existing 
        if (savedRow.getReceiptNumber() != null) {
            logger.error("Unable to delete TerminalPass with O.R. {} ", savedRow.getReceiptNumber());
            return new ResponseEntity(HttpStatus.CONFLICT);
        }
